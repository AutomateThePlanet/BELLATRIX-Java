...
import solutions.bellatrix.desktop.configuration.MobileSettings;
...
public class DriverService {
    ...
    public static WindowsDriver<WebElement> start(AppConfiguration configuration) {
        ...
        var MobileSettings = ConfigurationService.get(MobileSettings.class);
        var executionType = MobileSettings.getExecutionType();
        if (executionType.equals("regular")) {
            driver = initializeDriverRegularMode(MobileSettings.getServiceUrl());
        } else {
            var gridSettings = MobileSettings.getGridSettings().stream().filter(g -> g.getProviderName().equals(executionType.toLowerCase())).findFirst();
        ...
        driver.manage().timeouts().implicitlyWait(MobileSettings.getTimeoutSettings().getImplicitWaitTimeout(), TimeUnit.SECONDS);
