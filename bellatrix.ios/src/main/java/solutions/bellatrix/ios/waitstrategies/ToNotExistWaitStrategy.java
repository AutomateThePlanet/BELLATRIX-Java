...
public class ToNotExistWaitStrategy extends WaitStrategy {
    public ToNotExistWaitStrategy() {
        timeoutInterval = ConfigurationService.get(IOSSettings.class).getTimeoutSettings().getElementToNotExistTimeout();
    ...
    public <TFindStrategy extends FindStrategy> void waitUntil(TFindStrategy findStrategy) {
        Function<WebDriver, Boolean> func = (w) -> elementNotExists(DriverService.getWrappedIOSDriver(), findStrategy);
        waitUntil(func);
    }

    private <TFindStrategy extends FindStrategy> Boolean elementNotExists(IOSDriver<MobileElement> searchContext, TFindStrategy findStrategy) {
