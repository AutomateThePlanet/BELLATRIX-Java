package solutions.bellatrix.servicenow.pages.serviceNow;
...
import solutions.bellatrix.servicenow.pages.sections.availableSelectPopUp.AvailableSelectPopUpSection;
import solutions.bellatrix.servicenow.pages.sections.login.LoginSection;
import solutions.bellatrix.servicenow.pages.sections.personalizeColumn.PersonalizeColumnSection;
import solutions.bellatrix.servicenow.snSetupData.enums.ServiceNowLeftNavigationSection;
...
import solutions.bellatrix.servicenow.snSetupData.enums.ServiceNowWorkspaces;
...
public class ServiceNowPage extends WebPage<Map, Asserts> {
    ...
    @SneakyThrows
    public void openTablePageByDirectSearch(ServiceNowProjectTable serviceNowProjectTable) {
        browser().waitForAjax();
        ...
        if (isPolarisEnabled) {
            ...
            browser().waitUntilPageLoadsCompletely();
            for (int i = 0; i < 3; i++) {
                if (!browser().getUrl().contains(serviceNowProjectTable.getTable())) {
                    map().polarisMenuShadow().setText(String.format("%s.list", serviceNowProjectTable.getTable()) + Keys.ENTER);
                    browser().waitForAjax();
                    browser().waitUntilPageLoadsCompletely();
                } else {
                    break;
            ...
            while (!browser().getUrl().contains(serviceNowProjectTable.getTable())) {
                map().leftNavigatorFilter().setText(String.format("%s.list", serviceNowProjectTable.getTable()) + Keys.ENTER);
                browser().waitForAjax();
                browser().waitUntilPageLoadsCompletely();
            }
        }
...
//            browser().waitForReactPageLoadsCompletely();
    ...
    public <Item extends ServiceNowLeftNavigationItem> void openLeftNavigatorItem(ServiceNowLeftNavigationSection section, Item element) {
        openLeftNavigatorItem(Arrays.asList(section.getValue(), element.getText()));
        browser().waitUntilPageLoadsCompletely();
        browser().waitForAjax();
    }
    ...
    public void showColumnSearchRow() {
        map().showColumnSearchRow().toBeClickable().waitToBe();
        map().showColumnSearchRow().click();
    }

    public void hideColumnSearchRow() {
        map().hideColumnSearchRow().toBeClickable().waitToBe();
        map().hideColumnSearchRow().click();
    }

    public void hideColumnSearchByLeftMenu(ServiceNowLeftNavigationItem serviceNowLeftNavigationItem, ServiceNowLeftNavigationItem item) {
        switchToDefault();
        openFromLeftNavigation(serviceNowLeftNavigationItem, item);
        hideColumnSearchRow();
    }
    ...
    public String getDeviceInstallStatusValue() {
        return map().deviceInstallStatusField().getText();
    }
