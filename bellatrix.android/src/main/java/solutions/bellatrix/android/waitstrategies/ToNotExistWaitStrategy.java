...
public class ToNotExistWaitStrategy extends WaitStrategy {
    public ToNotExistWaitStrategy() {
        timeoutInterval = ConfigurationService.get(AndroidSettings.class).getTimeoutSettings().getElementToNotExistTimeout();
    ...
    public <TFindStrategy extends FindStrategy> void waitUntil(TFindStrategy findStrategy) {
        Function<WebDriver, Boolean> func = (w) -> elementNotExists(DriverService.getWrappedAndroidDriver(), findStrategy);
        waitUntil(func);
    }

    private <TFindStrategy extends FindStrategy> Boolean elementNotExists(AndroidDriver<MobileElement> searchContext, TFindStrategy findStrategy) {
        try {
